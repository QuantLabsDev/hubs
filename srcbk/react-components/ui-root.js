...
import { WalletSidebarContainer, WalletToolbarButtonContainer } from "./room/WalletSidebarContainer";
import { NFTSidebarContainer, NFTToolbarButtonContainer } from "./room/NFTSidebarContainer";
...
class UIRoot extends Component {
  ...
  updateGlobalWalletState = (status, dets) => {
    this.setState({
      globalWalletState: {
        status: status,
        dets: dets
  ...
  updateNftWalletState = (status, imgArray) => {
    this.setState({
      nftWalletState: {
        status: status,
        array: imgArray
          ...
          <div className={classNames(rootStyles)}>
            ...
            {this.props.hub && (
              <RoomLayoutContainer
                    ...
                    <>
                      {this.state.sidebarId === "wallet" && (
                        <WalletSidebarContainer
                          ...
                          globalWalletState={this.state.walletState}
                          updateGlobalWalletState={(status, dets) => this.updateGlobalWalletState(status, dets)}
                          setAccountData={(status, addr) => this.setAccountData(status, addr)}
                          accountData={this.state.accountData}
                        />
                      )}
                      {this.state.sidebarId === "nftwallet" && (
                        <NFTSidebarContainer
                          presences={this.props.presences}
                          occupantCount={this.occupantCount()}
                          canSpawnMessages={entered && this.props.hubChannel.can("spawn_and_move_media")}
                          ...
                          onClose={() => this.setSidebar(null)}
                          inputEffect={this.state.chatInputEffect}
                          nftWalletState={this.state.nftWalletState}
                          updateNftWalletState={(status, imgArray) => this.updateNftWalletState(status, imgArray)}
                          setAccountData={(status, addr) => this.setAccountData(status, addr)}
                          accountData={this.state.accountData}
                        />
                      )}
                ...
                toolbarMyCenter={
                  <>
                    <WalletToolbarButtonContainer
                      onClick={() => {
                        console.log("WalletToolbarButtonContainer Clicked");
                        this.toggleSidebar("wallet");
                      }}
                    />
                  </>
                }
                toolbarMyCenterTwo={
                  <NFTToolbarButtonContainer
                    onClick={() => {
                      console.log("WalletToolbarButtonContainer Clicked");
                      this.toggleSidebar("nftwallet");
                    }}
                  />
                }
                toolbarRight={
